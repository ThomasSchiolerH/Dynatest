...
const RidesMap: FC<IRidesMap> = ( { paths, selectedMetas, selectedMeasurements } ) => {
    ...
    /*
    ekki@dtu.dk: when loading rides concurrently, with using useMemo the trips are not properly updated
                 on the map
    ...
    */
    ...
    const memoPaths: { meas: MeasProperties, meta: RideMeta, bp: BoundedPath }[] = []
    selectedMeasurements.forEach( meas => {
        const { name } = meas;
        return selectedMetas.forEach( meta => {
            const { TaskId } = meta;
