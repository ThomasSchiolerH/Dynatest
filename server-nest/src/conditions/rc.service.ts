...
import { RoadConditions, Ways, ZoomConditions, Conditions, Conditions2 } from '../tables';
    ...
    async getConditions(
        ...
        try {
            let query = Conditions2(db)
                .select('coverage_values.id',
                    'type', 'value', 'std',
                    'start_time_utc', 'compute_time',
                    'task_id',
                    st.asGeoJSON('coverage.section_geom').as('section_geom'),
                    'IsHighway')
                .innerJoin('coverage', 'public.coverage.id', 'coverage_values.fk_coverage_id')
